...
func handleGroupBan(message MessageModel.Message) {
	if message.Sender.Role == "owner" || message.Sender.Role == "admin" {
		...
		if handler, exists := HTTPReq.ReqApiMap[ReqApiConst.SEND_GROUP_MSG]; exists {
			handler(ReqApiConst.SEND_GROUP_MSG, "[CQ:at,qq="+Int64toString(message.Sender.UserID)+"]"+"请求格式错误或用户不具备管理员权限", message.GroupID, MessageModel.NormalRespMessage(message.GroupID, "请求格式错误或用户不具备管理员权限"))
		}
		return
	}
...
func KickSomeBody(message MessageModel.Message) {
	if message.Sender.Role == "owner" || message.Sender.Role == "admin" {
		...
		for _, v := range QQNumberList {
			if StringToInt64(v) == message.SelfID {
				continue
			}
			if handler, exists := HTTPReq.ReqApiMap[ReqApiConst.SET_GROUP_KICK]; exists {

				handler(ReqApiConst.SET_GROUP_KICK, "踢人请求", message.GroupID, MessageModel.KickRespMessage(message.GroupID, StringToInt64(v), false))
			}
			return
		}
	} else {
...
func handleBan(message MessageModel.Message) {
	_, QQNumberList := AtQQNumber(message.RawMessage)
	BanReqFormatBoolen, _, Time := CheckBanFormat(message.RawMessage)
	if (message.Sender.Role == "owner" || message.Sender.Role == "admin") && BanReqFormatBoolen {
		for _, v := range QQNumberList {
			...
			if handler, exists := HTTPReq.ReqApiMap[ReqApiConst.SET_GROUP_BAN]; exists {
				...
				//
				//}
				handler(ReqApiConst.SET_GROUP_BAN, "禁言请求", message.GroupID, MessageModel.BanRespMessage(message.GroupID, StringToInt64(v), int64(Time)))
			}
			return
		}
	} else {
